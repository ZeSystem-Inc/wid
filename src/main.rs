...
fn main() {
    ...
    if args.len() < 2 {
        println!("🚀 wid - Windows Downloader v2.1.0");
        println!("Kullanım:");
        ...
        println!("  wid install <uygulama>    -> Uygulama kur (ör: wid install 7zip)");
    ...
    match command.as_str() {
        ...
        "info" => {
            ...
            if let Some(app) = apps_repo.iter().find(|a| a.name == query) {
                ...
                println!("  64 bit URL: {}", app.url64);
                println!("  64 bit Dosya: {}", app.file64);
                if let (Some(url), Some(file)) = (&app.url32, &app.file32) {
                    println!("  32 bit URL: {}", url);
                    println!("  32 bit Dosya: {}", file);
                } else {
                    println!("  32 bit: Mevcut değil");
                }
            } else {
        ...
        "search" => {
            ...
            for app in &apps_repo {
                if app.name.contains(&query) {
                    println!("  🔹 {} [{}] - {}", app.name, app.version, app.file64);
                    found = true;
                }
            }
            if !found {
        ...
        "install" => {
            ...
            let base_name = args[2].to_lowercase();

            if let Some(app) = apps_repo.iter().find(|a| a.name == base_name) {
                let arch = detect_arch();
                let (url, file_name) = if arch == "64" {
                    (app.url64.clone(), app.file64.clone())
                } else {
                    match (&app.url32, &app.file32) {
                        (Some(url), Some(file)) => (url.clone(), file.clone()),
                        _ => {
                            println!("⚠️  Bu paketin 32 bit sürümü yok. 64 bit sürüm kuruluyor...");
                            (app.url64.clone(), app.file64.clone())
                        }
                    }
                };

                println!("🎯 Kurulum: {} ({} bit)", app.name, arch);
                perform_installation_with_url(&app.name, &url, &file_name);
            } else {
                println!("❌ '{}' bulunamadı.", base_name);
            }
        }
        "upgrade" => {
            if args.len() < 3 {
                        ...
                        if let Some(installed_ver) = system::get_installed_version(&app.name) {
                            ...
                            let (url, file_name) = if arch == "64" {
                                ...
                                match (&app.url32, &app.file32) {
                                    (Some(url), Some(file)) => (url.clone(), file.clone()),
                                    _ => {
                                        println!("⚠️  32 bit sürüm yok, 64 bit kuruluyor...");
                                        (app.url64.clone(), app.file64.clone())
                                    }
                                }
                            };
...
fn perform_installation_with_url(app_name: &str, url: &str, file_name: &str) {
    let temp_dir = get_temp_dir();
    ...
    match network::download_file(url, file_path_str) {
        Ok(_) => {
            println!("⚙️ Kurulum başlatılıyor...");
            match system::install_package(file_path_str, app_name) {
                Ok(_) => println!("🎉 {} başarıyla kuruldu!", app_name),
