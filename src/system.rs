...
fn try_silent_install(file_path: &str, flag: &str, app_name: &str) -> bool {
    let output = Command::new("powershell")
        .args(&[
            "-Command",
            &format!(
                r#"$ErrorActionPreference = 'SilentlyContinue'; $p = Start-Process -FilePath '{}' -ArgumentList '{}' -Verb RunAs -Wait -WindowStyle Hidden -PassThru; if ($p.ExitCode -eq 0) {{ exit 0 }} else {{ exit 1 }}"#,
                file_path, flag
            )
        ])
        .output();
...
fn open_gui(file_path: &str, app_name: &str) -> Result<(), String> {
    ...
    if output.status.success() {
        ...
        let possible_paths = vec![
            ...
            format!("{}\\Program Files\\{}", program_files, app_name),
            format!("{}\\Program Files (x86)\\{}", program_files, app_name),
        ];

        for path in possible_paths {
            if std::path::Path::new(&path).exists() {
                println!("✅ Uygulama tespit edildi: {}", path);
                return Ok(());
            }
        }

        println!("❌ Kurulum tespit edilemedi (GUI tamamlanmamış olabilir).");
        ...
        "msi" => {
            ...
            let output = Command::new("powershell")
                .args(&[
                    "-Command",
                    &format!(
                        r#"$ErrorActionPreference = 'SilentlyContinue'; $p = Start-Process -FilePath 'msiexec' -ArgumentList '/i "{}" /quiet /norestart' -Verb RunAs -Wait -WindowStyle Hidden -PassThru; if ($p.ExitCode -eq 0) {{ exit 0 }} else {{ exit 1 }}"#,
                        file_path
                    )
                ])
                .output()
            ...
            if output.status.success() && get_installed_version(app_name).is_some() {
                println!("✅ MSI kurulum başarılı.");
                Ok(())
            } else {
                println!("⚠️  MSI kurulum başarısız.");
...
pub fn remove_package(app_name: &str) -> Result<(), String> {
                                    ...
                                    if !guid.is_empty() {
                                        let status = Command::new("powershell")
                                            .args(&[
                                                "-Command",
                                                &format!(
                                                    r#"$ErrorActionPreference = 'SilentlyContinue'; $p = Start-Process -FilePath 'msiexec' -ArgumentList '/x {{{}}} /quiet /norestart' -Verb RunAs -Wait -WindowStyle Hidden -PassThru; exit $p.ExitCode"#,
                                                    guid
                                                )
                                            ])
                                            .status()
                                        ...
                                        if status.success() {
                                            ...
                                            let status = Command::new("powershell")
                                                .args(&[
                                                    "-Command",
                                                    &format!(
                                                        r#"Start-Process -FilePath 'msiexec' -ArgumentList '/x {{{}}}' -Verb RunAs -Wait"#,
                                                        guid
                                                    )
                                                ])
                                                .status()
    ...
    Err(format!("'{}' için kaldırma komutu bulunamadı.", app_name))
}
...
fn get_available_version_from_repo(_app_name: &str) -> Option<String> {
    None
}